...
pair<int, int> pyflex_add_cloth_mesh_uvs(
...
/*
change from cloth-funnels we remove uvs
*/
...
pair<int, int> pyflex_emit_particles_box(py::array_t<float> position, py::array_t<float> orientation, py::array_t<int> dimension, py::array_t<float> velocity, float mass, float distance, int input_baseIndex, bool center)
{
    // position: float (initX, initY, initZ)
    auto ptr_position = (float *)position.request().ptr;
    Point3 p = Point3(ptr_position[0], ptr_position[1], ptr_position[2]);
    ...
    Vec3 offset = Vec3(0.0, 0.0, 0.0);
    if (center)
        offset = distance * Vec3(dimX - 1, 0.0, dimZ - 1) / 2.0;

    // velocity: float (vX, vY, vZ)
    auto ptr_velocity = (float *)velocity.request().ptr;
    Point3 velocity_point3 = Multiply(rotation_mat, Point3(ptr_velocity[0], ptr_velocity[1], ptr_velocity[2]));
    Vec3 velocity_vec3 = Vec3(velocity_point3.x, velocity_point3.y, velocity_point3.z);

    // int phase = NvFlexMakePhase(1, eNvFlexPhaseSelfCollide | eNvFlexPhaseFluid);
    int phase = NvFlexMakePhase(1, eNvFlexPhaseSelfCollide);
    float invMass = (dimX * dimZ) / mass;
    ...
    for (int y = 0; y < dimZ; ++y)
        ...
        {
            int index = baseIndex + y * dimX + x;
            Vec3 position = distance * Vec3(float(x), 0, float(y)) - offset;
            Point3 position_point3 = Point3(position.x, position.y, position.z);
            position_point3 = Multiply(transformation_mat, position_point3);
    ...
    NvFlexSetActive(g_solver, g_buffers->activeIndices.buffer, nullptr);
    if (input_baseIndex == -1)
        NvFlexSetActiveCount(g_solver, baseIndex + dimX * dimZ);
    NvFlexSetParticles(g_solver, g_buffers->positions.buffer, nullptr);
    NvFlexSetRestParticles(g_solver, g_buffers->restPositions.buffer, nullptr);
    NvFlexSetVelocities(g_solver, g_buffers->velocities.buffer, nullptr);
    NvFlexSetPhases(g_solver, g_buffers->phases.buffer, nullptr);
    return make_pair(baseIndex, dimX * dimZ);
}
...
void pyflex_empty(int scene_idx, py::dict scene_params)
{
    g_scene = scene_idx;
    g_selectedScene = g_scene;
    Init_Empty(g_selectedScene, scene_params);
}