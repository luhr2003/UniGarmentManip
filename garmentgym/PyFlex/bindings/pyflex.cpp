...
/*change from flingbot we append Softgym Clothes envs (indices5）
we append empty scene(indices6)
*/
void pyflex_init(bool headless=false, bool render=true, int camera_width=720, int camera_height=720) {
    ...
    g_scenes.push_back(new SoftgymClothes("Softgym Clothes"));
...
void pyflex_set_clothes(
    ...
    int thread_idx = 0){
    g_scene = Softgym_Clothes_Environment;
    g_selectedScene = g_scene;
    Init_clothes(g_selectedScene, scene_params, vertices,stretch_edges, bend_edges, shear_edges, faces,
         true, thread_idx);
}
...
void pyflex_set_camera(py::dict scene_params)
{
    if (g_render)
    {
...
PYBIND11_MODULE(pyflex, m) {
    ...
    m.def("set_clothes", &pyflex_set_clothes,
          py::arg("scene_idx") = Softgym_Clothes_Environment,
    ...
    m.def("add_cloth_mesh_uvs",
          &pyflex_add_cloth_mesh_uvs,
          "Add cloth (mesh)",
          py::arg("position"),
          py::arg("verts"),
          py::arg("faces"),
    ...
    m.def("empty",&pyflex_empty, "Create empty Scene");
    m.def("set_camera", &pyflex_set_camera, "Set camera");
}
