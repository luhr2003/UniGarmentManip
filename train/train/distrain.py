...
from train.dataloader.dis_dataloader import disDataset
...
def train(config:Config,writer:SummaryWriter,checkpoint_dir:str,log_train_path:str,log_val_path:str):
    ...
    train_dataset=disDataset(deform_path=deform_path,object_path=object_path,config=config,flag="train")
    val_dataset=disDataset(deform_path=deform_path,object_path=object_path,config=config,flag="val")
    ...
    for epoch in range(config.train_config.epoch):
        ...
        total_loss=0
        for i,(pc1,pc2,correspondence,negative_info) in enumerate(train_dataloader):
            ...
            negative_info=negative_info.to(config.train_config.device)
            ...
            # negative_info batchsize*2*num_correspondence*num_negative
            # negative_id batchsize*num_correspondence*num_negative
            # negative_distance batchsize*num_correspondence*num_negative
            negative_id=negative_info[:,0]
            negative_distance=negative_info[:,1]
            negative_id_1=negative_id[:,:,:config.train_config.num_negative//2].long()
            negative_id_2=negative_id[:,:,config.train_config.num_negative//2:].long()
            negative_distance_1=negative_distance[:,:,:config.train_config.num_negative//2]
            negative_distance_2=negative_distance[:,:,config.train_config.num_negative//2:]
            negative_id_1=negative_id_1.reshape(batchsize,num_correspondence*config.train_config.num_negative//2)
            negative_id_2=negative_id_2.reshape(batchsize,num_correspondence*config.train_config.num_negative//2)
            negative_feature_1=pc1_output.gather(1,negative_id_1.unsqueeze(2).expand(-1,-1,feature_dim))
            negative_feature_2=pc2_output.gather(1,negative_id_2.unsqueeze(2).expand(-1,-1,feature_dim))
            # negative_feature_1 batchsize*num_correspondence*num_negative//2*feature_dim
            # negative_feature_2 batchsize*num_correspondence*num_negative//2*feature_dim
            negative_feature_1=negative_feature_1.reshape(batchsize,num_correspondence,config.train_config.num_negative//2,feature_dim)
            negative_feature_2=negative_feature_2.reshape(batchsize,num_correspondence,config.train_config.num_negative//2,feature_dim)
            #negative_1 batchsize*num_correspondence*num_negative//2*feature_dim
            #negative_2 batchsize*num_correspondence*num_negative//2*feature_dim
            negative_1=negative_feature_1*(negative_distance_1.unsqueeze(3).expand(-1,-1,-1,feature_dim))
            negative_2=negative_feature_2*(negative_distance_2.unsqueeze(3).expand(-1,-1,-1,feature_dim))
            negative=torch.cat([negative_1,negative_2],dim=2)
        ...
        with torch.no_grad():
            ...
            for i,(pc1,pc2,correspondence) in enumerate(val_dataloader):
                ...
                # print("original shape",pc1.shape,pc2.shape,correspondence.shape)
                #pc1_output batchsize*num_points*feature_dim
                #pc2_output batchsize*num_points*feature_dim
                pc1_output=model(pc1)
                pc2_output=model(pc2)
                feature_dim=pc1_output.shape[2]
                # print("after model shape",pc1_output.shape,pc2_output.shape)
                ...
                negative_id=negative_info[:,0]
                negative_distance=negative_info[:,1]
                nagative_id_1=negative_id[:,:,:config.train_config.num_negative//2]
                nagative_id_2=negative_id[:,:,config.train_config.num_negative//2:]
                nagative_distance_1=negative_distance[:,:,:config.train_config.num_negative//2]
                nagative_distance_2=negative_distance[:,:,config.train_config.num_negative//2:]
                negative_id_1=negative_id_1.reshape(batchsize,num_correspondence*config.train_config.num_negative//2)
                negative_id_2=negative_id_2.reshape(batchsize,num_correspondence*config.train_config.num_negative//2)
                negative_feature_1=pc1_output.gather(1,negative_id_1.unsqueeze(2).expand(-1,-1,feature_dim))
                negative_feature_2=pc2_output.gather(1,negative_id_2.unsqueeze(2).expand(-1,-1,feature_dim))
                ...
                #negative_1 batchsize*num_correspondence*num_negative//2*feature_dim
                #negative_2 batchsize*num_correspondence*num_negative//2*feature_dim
                negative_1=negative_feature_1*(nagative_distance_1.unsqueeze(3).expand(-1,-1,-1,feature_dim))
                negative_2=negative_feature_2*(nagative_distance_2.unsqueeze(3).expand(-1,-1,-1,feature_dim))
                negative=torch.cat([negative_1,negative_2],dim=2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs_dir',type=str,default="./logs/dis_nce/")
    parser.add_argument('--prefix',type=str,default="dis_nce")
    parser.add_argument('--resume',type=bool,default=False)
    parser.add_argument('--resume_path',type=str,default="")
    parser.add_argument("--device",type=str,default="cuda:0")
    parser.add_argument("--deform_path",type=str,default="/home/isaac/correspondence/tshirt_move")
    parser.add_argument("--object_path",type=str,default="/home/isaac/correspondence/UniGarmentManip/garmentgym/cloth3d/train")
